  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('weekly');

        const virtualModuleId = 'weekly:config';
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Weekly \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Weekly config has been loaded.`);
        }
      },
